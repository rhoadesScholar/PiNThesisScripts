classdef rlMDPEnv_Jeff < rl.env.MATLABEnvironment
...
%   See also: createGridWorld, createMDP
    ...
    %% Public Methods
    methods
        function obj = rlMDPEnv_Jeff(MDP)
    ...
    methods 
        ...
        function [Observation,Reward,isTerminal,LoggedSignals] = step(this,Action,simCount)
            LoggedSignals = [];
            Action = idx2action(this.Model,Action);
            if ~isempty(this.StepFcn)
%                 feval(this.StepFcn(this, Action));
                this.StepFcn(this, Action,simCount)
            end
