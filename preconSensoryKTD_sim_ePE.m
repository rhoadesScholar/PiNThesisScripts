function [results, model, missTrials] = preconSensoryKTD_sim_ePE(preconProb, showStages, varCon, param)
...
%     stage = [stage; 12*ones(size(F12,1))];
    ...
    [results, model] = kalmanSensoryTD_ePE(F,r,stage,trial,param);
...
function plotResults(results, showStages)
    if length(showStages) > 1
        colors = linspecer(length(showStages));
        getColor = @(s,t) colors(showStages == s,:);
    else
        colors = linspecer(size(results(showStages(1)).rhat, 1), 'sequential');
        getColor = @(s,t) colors(t,:);
    end
    figure;
...
%         labels(end+1) = {sprintf('Stage #%i', s)};
        for t = 1:size(results(s).rhat, 1)%trial
            subplot(2,2,1)
            hold on; axis tight        
            if s==showStages(1) && t ==1
                title('Reward Prediction');
            end
            p = plot(results(s).rhat(t,:), 'Color', getColor(s,t), 'DisplayName', sprintf('Stage #%i', s));
            if t ~= 1
        ...
        for t = 1:size(results(s).dt, 1)%trial
            ...
            p = plot(results(s).dt(t,:), 'Color', getColor(s,t), 'DisplayName', sprintf('Stage #%i', s));
            if t~=1
                p.Annotation.LegendInformation.IconDisplayStyle = 'off';
            else
                p.LineWidth = 1;
        ...
        for t = 1:size(results(s).eDt, 1)%trial
            subplot(2,2,3)
            hold on; axis tight        
            if s==showStages(1) && t ==1
                title('Estimated Prediction Error')
            end
            p = plot(results(s).eDt(t,:), 'Color', getColor(s,t), 'DisplayName', sprintf('Stage #%i', s));
            if t~=1
                p.Annotation.LegendInformation.IconDisplayStyle = 'off';
            else
                p.LineWidth = 1;
        ...
        for t = 1:size(results(s).mDt, 1)%trial
            subplot(2,2,4)
            hold on; axis tight               
            if s==showStages(1) && t ==1
                title('Meta-Prediction Error')
            end
            p = plot(results(s).mDt(t,:), 'Color', getColor(s,t), 'DisplayName', sprintf('Stage #%i', s));
            if t~=1
                p.Annotation.LegendInformation.IconDisplayStyle = 'off';
            else
                p.LineWidth = 1;
